...
## read in mapping file (present study + HMP)
...
otu.ger.genus <- read.table('data/tax_16S_uparse/count_tables/ger_uparse_merged_taxonomy_summaries/ger_uparse_merged_L6_filtered_2samples.txt',
...
## merge OTU data for present study and HMP
...
####################################
## Combine merged present study/Wood/Barberan data with HMP source

## read in merged OTU table (L6, genus)
## ** Note: this includes samples (present study) filtered to OTUs that occur only in 2+ samples,
...
pick.ger <- which(map.ger.wood.barb$TITLE == 'University mixed-use facility')
